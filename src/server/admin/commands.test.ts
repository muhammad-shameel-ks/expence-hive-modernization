...
describe("role capabilities", () => {
  ...
  it("queries the impact without applying anything", async () => {
    const { admin, store } = buildAdmin(
      [CUSTOM_REVIEWER_ROLE],
      [claimWithPendingStepAt("role-reviewer", { title: "Reviewer claim two" })],
    );
    ...
    expect(impact).toEqual({
      removedActionPrivileges: ["canApprove"],
      pendingClaims: [expect.objectContaining({ title: "Reviewer claim two" })],
    });
    const roles = await store.listRoles("org-1");
    expect(roles.find((candidate) => candidate.id === "role-reviewer")?.capabilities).toEqual(
      MANAGER_CAPABILITIES,
    );
  });

  it("ignores claims whose pending step targets another role, and draft and finished claims", async () => {
    const { admin } = buildAdmin(
      [CUSTOM_REVIEWER_ROLE],
      [
        claimWithPendingStepAt("role-other"),
        claimWithPendingStepAt("role-reviewer", { status: "paid" }),
        claimWithPendingStepAt("role-reviewer", { status: "draft", steps: [] }),
      ],
    );
