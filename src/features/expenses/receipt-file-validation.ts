...
// The client gate mirrors the server's authoritative set: only JPEG, PNG,
// and PDF pass. An empty declared type and application/octet-stream (the
// wire form of an absent declaration in multipart bodies) are allowed; the
// server sniffs magic bytes and is authoritative for those files.
const RECEIPT_TYPES = new Set(["image/jpeg", "image/png", "application/pdf", "application/octet-stream"]);

export function receiptValidationError(file: File): string | null {
  if (file.type !== "" && !RECEIPT_TYPES.has(file.type)) {
    return "Receipts must be a JPEG, PNG, or PDF file."
  }
